...
//            return this.$menu && this.$menu.is(":visible");
		...
		setEntry: function(cmd, titleOrData){
			...
			if(typeof titleOrData === "string"){
				...
				$.moogle.contextmenu.createEntryMarkup(titleOrData, $parent, true); // fandm add third param
			}
		},
...
$.extend($.moogle.contextmenu, {
	...
	createEntryMarkup: function(entry, $parentLi, flag){ // fandm add third param
		var $a = null;
		...
		if( !/[^\-\u2014\u2013\s]/.test( entry.title ) ){
			...
			if (entry.children && $.isArray(entry.children) && entry.children.length > 0 && flag) { // fandm 07.12.13
				$a.attr('aria-haspopup', 'true');
				$a.attr('role','menuitem');
				$a.attr('tabindex','-1');
				$a.addClass('ui-corner-all');
				$a.prepend($("<span class='ui-menu-icon ui-icon ui-icon-carat-1-e'>"));
				var $ul = $("<ul>");
				$ul = $.moogle.contextmenu.createMenuMarkup(entry.children, $ul); 
				$ul.menu().attr('aria-expanded','false').attr('aria-hidden','true').css('display','none').appendTo($parentLi);
			}
		}
		return $a;
	},
	...
	createMenuMarkup: function(options, $parentUl){
		...
		for(i = 0; i < options.length; i++){
			...
			$.moogle.contextmenu.createEntryMarkup(menu, $li, false); // fandm add third param
